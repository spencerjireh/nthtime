...
function loadChallenges(): { pack: string; challenge: ChallengeFile }[] {
  ...
  for (const packName of packDirs) {
    ...
    const files = readdirSync(challengeDir).filter((f) => f.endsWith('.json')).sort();
...
async function main() {
  ...
  console.log(`Benchmarking ${challenges.length} challenges across ${new Set(challenges.map((c) => c.pack)).size} packs\n`);
  ...
  const header = `${'Pack'.padEnd(maxPack)}  ${'Challenge'.padEnd(maxTitle)}  ${'Time'.padStart(8)}  Status`;
  console.log(header);
  ...
  for (const r of results) {
    ...
    console.log(`${r.pack.padEnd(maxPack)}  ${r.challenge.padEnd(maxTitle)}  ${time}  ${status}${slow}`);
  }
  ...
  console.log(`Challenges: ${results.length} | Passed: ${results.filter((r) => r.passed).length} | Slow (>${MAX_MS}ms): ${slowCount}`);
