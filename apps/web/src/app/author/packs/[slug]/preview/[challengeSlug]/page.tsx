...
import { authorChallengeEditHref } from '@/lib/routes';
...
export default function PreviewRoute({ params }: PreviewRouteProps) {
  ...
  return <PreviewLoader packSlug={slug} challengeSlug={challengeSlug} />;
}
...
function PreviewLoader({
  ...
  return (
    <PreviewInner
      packSlug={packSlug}
      challengeId={challengeSummary._id}
    ...
    <div className="flex h-[calc(100vh-3.5rem)] flex-col">
      <div className="flex items-center gap-2 border-b border-border bg-amber-500/10 px-4 py-1.5">
        ...
        <Button variant="ghost" size="sm" className="ml-auto h-6 text-xs" asChild>
          <Link href={authorChallengeEditHref(packSlug, challengeSlug)}>
